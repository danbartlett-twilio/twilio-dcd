<script>
  ...
  export default {
    ...
    setup() {
      ...
      // GET LIST DETAILS FROM Sync Twilio Table Main
      const getListData = async () => {
        
        try {
          if (syncStore?.twilioTableSID !== null) {
            let url = `${import.meta.env.VITE_DATA_URL}/sync/list-item-get-array?targetList=${syncStore.twilioTableSID}&filterField=listSid&filterFieldValue=${tableSid}`;
      ...
      const getListItems = async () => {
        ...
        try {          
          let url = `${import.meta.env.VITE_DATA_URL}/sync/list-item-get-array?targetList=${tableSid}&ts=${Date.now()}`;
          const res = await fetch(url, { method: "GET", cache: "no-store", headers: {'Content-type': 'application/json'} });
          if (res.ok) {
            let r = await res.json();
      ...
      const saveNewItemObject = async (p) => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync/list-item-create?targetList=${tableSid}`;
      ...
      const deleteItem = async (i) => {
        ...
        let url = `${import.meta.env.VITE_DATA_URL}/sync/list-item-delete?targetList=${tableSid}&listItemIndex=${i}`;
